...
<template>
    <AppLayout title="Competição">
        <header class="flex flex-col items-center gap-4 p-4 text-center">
            <h2
                class="bg-2023-red shadow-2023-teal w-fit border border-solid border-black p-3 text-center text-2xl font-bold text-white shadow-md xl:text-3xl 2xl:text-4xl"
            >
                {{ competition.name }}
            </h2>
            <span
                class="text-2023-red-dark inline-flex w-3/4 justify-center text-xl font-bold"
                >{{ competition.theme }}</span
            >
            <span
                class="text-2023-teal inline-flex justify-center text-xl font-bold"
                >{{
                    formattedDate(
        ...
        <section
            class="bg-2023-teal-dark relative mt-5 border-t border-black text-justify"
        >
            <h2
                class="bg-2023-red shadow-2023-bg absolute -top-9 left-1/2 -translate-x-1/2 transform border border-solid border-black p-3 text-2xl font-bold text-white shadow-md xl:text-3xl 2xl:text-4xl"
            >
                Regulamento
            </h2>
            <div
        ...
        <template v-if="isOver">
            ...
            >
                <template v-if="competition.teams && competition.teams.length">
                    ...
                    <span class="text-2023-teal text-2xl font-bold">
                        Equipas: {{ competition.teams?.length }}
                    </span>
                    <div
                        class="shadow-2023-teal flex w-4/5 flex-col border border-black shadow-lg md:w-1/2"
                    >
                        <div
                            v-for="team in competition.teams ?? []"
                            :key="team.id"
                            class="even:bg-2023-orange inline-flex w-full justify-between gap-1 p-4 text-lg even:bg-opacity-20"
                        >
                            <img
                                :src="
        ...
        <template v-else-if="!isAdmin($page.props.auth.user)">
            ...
            >
                ...
                <div
                    class="bg-2023-bg mx-10 flex h-full flex-1 flex-col items-center p-3 text-center text-3xl font-bold text-white sm:p-12"
                >
                    <PrimaryButton
                        color="orange"
                        shadow="teal"
            ...
            >
                <h1
                    class="text-2023-red absolute top-[1rem] m-4 flex justify-center p-3 text-4xl font-black"
                >
                    Vamos a isto?
                </h1>
                ...
                <a
                    ...
                    class="bg-2023-teal shadow-2023-orange relative mt-5 content-center justify-center border border-black px-8 py-2 text-center text-2xl font-semibold text-white transition-shadow hover:shadow-md active:shadow-none"
                >
                    Participar!
                </a>
            </section>
